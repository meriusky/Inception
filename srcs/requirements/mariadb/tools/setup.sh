...
# Minimal settings
DATADIR="/home/mysqluser/data"  # Where MariaDB will store data
OSUSER="mysqluser"              # Linux user that runs the MariaDB
BIND="0.0.0.0"                  # makes MariaDB listen on all IPs(so WP can connect)
MARKER="$DATADIR/.initialized"  # hidden file used as a flag to know in initialization is done

# Ensure dirs and ownership
mkdir -p /run/mysqld "$DATADIR"
chown -R "$OSUSER:$OSUSER" /run/mysqld "$DATADIR" #gives ownership to mysqluser
...
INIT_SQL=""
if [ ! -f "$MARKER" ]; then 
  INIT_SQL="/tmp/init.sql" # Where all commands will be writen
  : > "$INIT_SQL"
...
# Create admin user, allows login both from localhost and gives dull privileges
...
# Create database and normal user if vars exist
  ...
  echo "FLUSH PRIVILEGES;" >> "$INIT_SQL" #FLUSH PRIVILEGES; tells MariaDB to reload user table changes
